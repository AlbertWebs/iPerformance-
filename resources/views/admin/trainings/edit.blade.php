...
<div class="max-w-3xl">
    <nav class="mb-8 flex flex-wrap items-center gap-2 text-sm">
        ...
        <span class="truncate font-medium text-slate-700" title="{{ $training->title }}">{{ Str::limit($training->title, 32) }}</span>
        <svg class="h-4 w-4 shrink-0 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
    ...
    <div class="mb-10 flex flex-col gap-4 sm:flex-row sm:items-start sm:gap-6">
        ...
        <div class="min-w-0 flex-1">
            <h2 class="text-xl font-bold text-slate-900">Edit training</h2>
            <p class="mt-1 text-base font-medium text-slate-800" title="{{ $training->title }}">{{ Str::limit($training->title, 60) }}</p>
            <div class="mt-2 flex flex-wrap items-center gap-x-3 gap-y-1 text-sm text-slate-600">
                <span>{{ $training->start_date->format('M j') }} – {{ $training->end_date->format('M j, Y') }}</span>
                @if($training->location)<span>{{ $training->location }}</span>@endif
                @if($training->price)<span>KES {{ number_format($training->price) }}</span>@endif
                @if($training->category)<span class="rounded-full bg-slate-100 px-2.5 py-0.5 text-slate-600">{{ $training->category->name }}</span>@endif
                @if($training->is_active)<span class="text-emerald-600">Active</span>@else<span class="text-slate-500">Inactive</span>@endif
            </div>
            <a href="{{ route('trainings.show', $training) }}" target="_blank" rel="noopener" class="mt-3 inline-flex items-center gap-1.5 text-sm font-medium text-primary hover:text-primary-hover">
    ...
    <form method="POST" action="{{ route('admin.trainings.update', $training) }}" enctype="multipart/form-data" id="training-edit-form">
        @csrf
        @method('PUT')
        @include('admin.trainings.form', ['training' => $training, 'categories' => $categories])
    ...
    <div class="fixed bottom-0 left-0 right-0 z-20 border-t border-slate-200 bg-white/95 px-8 py-4 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] backdrop-blur sm:left-64">
        <div class="mx-auto flex max-w-3xl flex-wrap items-center justify-between gap-4">
            <div class="flex flex-wrap items-center gap-3">
                ...
                <a href="{{ route('trainings.show', $training) }}" target="_blank" rel="noopener" class="inline-flex items-center gap-1.5 text-sm font-medium text-primary hover:text-primary-hover">
                    <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"/></svg>
                    View on site
                </a>
            </div>
            <div class="flex flex-wrap gap-3">
                ...
                <button type="submit" form="training-edit-form" class="inline-flex items-center gap-2 rounded-xl bg-primary px-5 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-primary-hover">
