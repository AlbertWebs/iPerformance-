...
<body class="font-sans antialiased text-slate-800 bg-white min-h-screen flex flex-col">
                ...
                <div class="flex flex-wrap items-center justify-center gap-x-6 gap-y-1 text-xs sm:justify-between sm:text-sm">
                    <div class="flex flex-wrap items-center justify-center gap-x-6 gap-y-1 sm:justify-start">
                        ...
                        @if(!empty($topContact['address']))
                            <a href="https://share.google/ppgbVVaT316NVsBAq" target="_blank" rel="noopener" class="inline-flex items-center gap-1.5 transition hover:text-primary">
    ...
    <nav class="fixed bottom-0 left-0 right-0 z-30 grid grid-cols-4 gap-px border-t border-slate-200 bg-slate-100 md:hidden" aria-label="Primary">
        ...
        <a href="{{ route('certifications.index') }}" class="flex flex-col items-center justify-center gap-1 py-3 text-slate-600 transition {{ request()->routeIs('certifications.*') ? 'bg-white text-primary font-semibold' : 'bg-white hover:bg-slate-50 hover:text-primary' }}">
            ...
            <span class="text-xs">Certifications</span>
        </a>
    </nav>

    {{-- Go to top button (bottom right) --}}
    <button type="button" id="go-to-top" aria-label="Scroll to top" class="go-to-top-btn fixed bottom-20 right-4 z-40 flex h-12 w-12 items-center justify-center rounded-full bg-primary text-white shadow-lg transition-all duration-300 hover:bg-primary-hover focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 opacity-0 pointer-events-none md:bottom-6 md:right-6">
